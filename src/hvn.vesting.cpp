...
void vesting::init_balance(name user) {
...
void vesting::check_asset(asset quantity) {
    check(quantity.is_valid(), "invalid asset");
    check(quantity.symbol == seeds_symbol, "invalid asset");
}


void vesting::createaux(name sponsor, name beneficiary, asset quantity, uint64_t vesting_date, uint8_t type){
    // check if the sponsor has enough balance
    auto it_b = sponsors.find(sponsor.value);
    
    check(it_b != sponsors.end(), "vesting: The user " + sponsor.to_string() + " does not have a balance entry");
    check(it_b -> balance >= quantity, "vesting: The sponsor " + sponsor.to_string() + " does not have enough balance");
...
void vesting::cancelaux(name sponsor, name beneficiary) {
    ...
    check(it != escrows_by_sponsor.end(), "vesting: The user " + sponsor.to_string() + " does not have a escrow entry");
...
void vesting::ontransfer(name from, name to, asset quantity, string memo) {
    if(to == get_self()) {
        init_balance(from);
        init_balance(to);
...
void vesting::withdraw(name sponsor, asset quantity) {
    ...
    check(it != sponsors.end(), "vesting: the user " + sponsor.to_string() + " does not have a balance entry");
    check(it -> balance >= quantity, "vesting: the sponsor " + sponsor.to_string() + " does not have enough balance");
...
void vesting::create(name sponsor, name beneficiary, asset quantity, uint64_t vesting_date) {
    require_auth(sponsor);
...
void vesting::createescrow(name sponsor_bucket, name beneficiary_org, asset quantity, uint64_t vesting_date) {
...
void vesting::cancel(name sponsor, name beneficiary) {
    require_auth(sponsor);
    cancelaux(sponsor, beneficiary);
}


void vesting::cancelescrow(name sponsor, name beneficiary) {
    require_auth(sponsor);
    cancelaux(sponsor, beneficiary);
}


void vesting::claim(name beneficiary) {
    ...
    check(it != escrows_by_beneficiary.end(), "vesting: The user " + beneficiary.to_string() + " does not have and escrow entry");
    ...
    check(total_quantity > asset(0, seeds_symbol), "vesting: The beneficiary does not have any available escrows, try to claim them after their vesting date");
