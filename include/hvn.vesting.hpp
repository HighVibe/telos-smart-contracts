...
CONTRACT vesting : public contract {
    public:
        ...
        vesting(name receiver, name code, datastream<const char*> ds)
...
extern "C" void apply(uint64_t receiver, uint64_t code, uint64_t action) {
  if (action == name("transfer").value && code == contracts::token.value) {
      execute_action<vesting>(name(receiver), name(code), &vesting::ontransfer);
  } else if (code == receiver) {
      switch (action) {
          EOSIO_DISPATCH_HELPER(vesting, (reset)(create)(createescrow)(cancel)(cancelescrow)(claim)(withdraw))
      }
  }
}
